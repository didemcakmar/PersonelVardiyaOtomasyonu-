...
namespace PersonelVardiyaOtomasyonu
{
	public partial class vardiya : Form
	{
		...
		{
			var vardiya = _context.vardiya.ToList();
		...
		private void vardiyaDataTable_CellContentClick(object sender, DataGridViewCellEventArgs e)
		{
			if (e.RowIndex >= 0 && e.RowIndex < vardiyaDataTable.Rows.Count)
			{
				...
				string vard_bas_saat = selectedRow.Cells["vard_bas_saat"].Value?.ToString();
				baslangic_saati_TextBox.Text = vard_bas_saat;

				string vard_bit_saat = selectedRow.Cells["vard_bit_saat"].Value?.ToString();
				bitis_saati_TextBox.Text = vard_bit_saat;

				string vard_gun = selectedRow.Cells["vard_gun"].Value?.ToString();
				gun_TextBox.Text = vard_gun;

				string vard_konum = selectedRow.Cells["vard_konum"].Value?.ToString();
				lokasyon_comboBox.Text = vard_konum;
		...
		private void ButtonGuncelle_Click(object sender, EventArgs e)
		{
			if (vardiyaDataTable.SelectedRows.Count > 0)
			{
				...
				try
				{
					var selectedVardiya = _context.vardiya.FirstOrDefault(v => v.id == selectedId);
					...
					if (selectedVardiya != null)
					{
						...
						if (int.TryParse(sicil_no_TextBox.Text, out int sicil))
						{
							selectedVardiya.pers_sicil = sicil;
						}
						else
						{
				...
				if (result == DialogResult.Yes)
				{
					...
					try
					{
						...
						if (selectedVardiya != null)
						{
							...
							_context.vardiya.Remove(selectedVardiya);
		...
		private void ButtonReset_Click(object sender, EventArgs e)
		{
			...
			baslangic_saati_TextBox.Clear();
			bitis_saati_TextBox.Clear();
			gun_TextBox.Clear();
			vardiyaDataTable.ClearSelection();
